...
REM The change detector writes below .dart_tool before the first `dart pub get`.
REM Create it explicitly so a clean Windows Android build cannot silently skip
REM Cargo after redirecting into a directory that does not exist yet.
...
if not exist "%PRECOMPILED%" (
    ...
    REM Cache cleanup removes this runner's package config, but its packages
    REM normally remain in the shared Pub cache. Resolve offline first so an
