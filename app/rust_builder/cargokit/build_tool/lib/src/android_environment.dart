...
class AndroidEnvironment {
  ...
  Future<Map<String, String>> buildEnvironment() async {
    ...
    // The NDK ships target-aware clang launchers (for example
    // x86_64-linux-android21-clang.cmd). Using Cargokit's Dart launcher as the
    // linker recursively starts dependency discovery for every cdylib link on
    // Windows. A cold build can consequently look frozen for tens of minutes
    // and may finish only the static archive. The NDK launcher supplies the
    // same --target argument directly without re-entering Cargokit.
    final linkerExtension = Platform.isWindows ? '.cmd' : '';
    final linkerValue = path.join(
      toolchainPath,
      '${target.rust}$minSdkVersion-clang$linkerExtension',
    );
    if (!File(linkerValue).existsSync()) {
      throw Exception('Failed to find linker for $target at $linkerValue');
    }
